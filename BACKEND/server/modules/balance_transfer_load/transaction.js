...
(() => {
    module.exports = async (call, res) => {
        try {
            let existingtoaccountNumber = await mysqlHelper.format(
                `select * from db_balance_humanity.customer_account_information where account_number = ${call.body.toAccountNumber}   `
            );
        ...
        WHERE
           ai.account_number ="${call.body.toAccountNumber}"`);
                ...
                if (victimValidationResult[0].customer_type == "NormalCustomer") {
                    return res
                        ...
                        .json({
                            ...
                            message: "Verify Kyc of victim to proceed transctions!",
                        });
                } else if (victimValidationResult[0].fullName != call.body.accountName) {
                    return res
                        .status(200)
                        .json({status: httpStatus.BAD_REQUEST, message: "account name doesnt matched!"});
                }
            }

            let userInfo = await mysqlHelper.format(
                `select * from db_balance_humanity.balance_humanity_users where uuid = "${call.body.user.uuid}"`
            );
            ...
            if (userResult && userResult.length > 0) {
                let existingAmount = await mysqlHelper.format(
                    `select * from db_balance_humanity.customer_account_information where customer_id = ${userResult[0].id}   `
                );
                ...
                if (existingAmountResult && existingAmountResult.length > 0) {
                    ...
                    if (fromAccountNumber == call.body.toAccountNumber) {
                        return res
                            ...
                            .json({
                                status: httpStatus.BAD_REQUEST,
                                message: "from account number and to account number cannt be same",
                            });
                    }
                    let existingAmountReal = await mysqlHelper.format(
                        `select * from db_balance_humanity.customer_account_information where account_number = ${fromAccountNumber}   `
                    );
                            ...
                            if (logInsertRes && logInsertRes.affectedRows > 0) {
                                return res
                                    ...
                                    .json({
                                        ...
                                        responseData: JSON.stringify(logdataTransaction),
                                    });
                            }
                        }
                    }
