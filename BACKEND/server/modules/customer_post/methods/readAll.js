...
const { readAllCustomerPostSql } = require("../sql");
...
module.exports = async (call,res) => {
    try {
        ...
        response = await readAllCustomerPostSql(call.body);
        if (response.status !== httpStatus.OK) {
            return res.status(400).json({ status: httpStatus.BAD_REQUEST,message: response.message });
            // return response;
        }
        return res.status(200).json({status: httpStatus.OK, message: response.data });
        // return response;
    } catch (error) {
