            ...
            if (userExistCheckResult && userExistCheckResult.length > 0) {
                ...
                if (match) {
                    ...
                    if (redisValue && redisValue.accessToken) {
                        ...
                        if(accessToken){
                            const decodedAccess = await jwtHelper.verifyJWTToken(accessToken,process.env.ACCESS_TOKEN_SECRET_KEY);
                            ...
                            const decodedRefresh = await jwtHelper.verifyJWTToken(refreshTokenFromRedis,process.env.REFRESH_TOKEN_SECRET_KEY);
                            if(decodedRefresh && decodedRefresh.success == true){
                                let accessTokenFromRefreshToken = jwtHelper.generateJWTAccessToken(decodedRefresh.data.user)
                    ...
                    if (jwtAccessToken.success && jwtRefreshToken.success) {
                        ...
                        await setValues(customerData.email,{accessToken:jwtAccessToken,refreshToken:jwtRefreshToken,message:"login successfully!",customerData: JSON.stringify(customerData) });
                        // await setValues(customerData.email,jwtRefreshToken);

                        return res.status(200).json({ accessToken: jwtAccessToken, refreshToken: jwtRefreshToken, message: "login successfully!", customerData: JSON.stringify(customerData) });
                    }
