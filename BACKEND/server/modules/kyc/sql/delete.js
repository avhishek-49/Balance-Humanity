...
(() => {
  module.exports = async (call, res) => {
    try {
      ...
      let query = await mysqlHelper.format(`DELETE from db_balance_humanity.balance_humanity_kyc where mobile_number = ?`, [call.mobile_number])
      const [result] = await mysqlHelper.query(query);

     

      if (result && result.affectedRows > 0) {
        return response = { status: httpStatus.OK, message: "User deleted successfully"}
      }
      return response
    } catch (error) {
      console.error(error);
