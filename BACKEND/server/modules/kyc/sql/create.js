...
(() => {
  module.exports = async (call, res) => {
    try {
      ...
      let query = await mysqlHelper.format(`INSERT IGNORE INTO db_balance_humanity.balance_humanity_kyc SET ?`, [insertObj])
      ...
      if (result && result.affectedRows > 0) {
        return response = { status: httpStatus.OK, message: "KYC successfully submitted wait until verification!"}
      }
    } catch (error) {
      console.error(error);
