((redisHelper) => {
    ...
    redisHelper.getCachedStringDataPromise = (_keyData, res, next) => {
      return new Promise((resolve, reject) => {
        client.get(_keyData, (err, data) => {
          if (!err && data !== null) {
            return resolve({
    ...
    redisHelper.getCachedStringData = (_keyData, res, next) => {
      client.get(_keyData, (err, data) => {
        if (!err && data !== null) {
    ...
    redisHelper.setDataToCache = (_keyData, data, ttl = false) => {
      return new Promise((resolve, reject) => {
        ...
        client.setex(_keyData, parseInt((ttl ? parseFloat(ttl) : parseFloat(process.env.REDIS_CACHE_EXPIRES)) * 60), storeData, (err, data) => {
          if (!err && data !== null) {
            return resolve(data);
          } else {
            console.log({ err });
    ...
    redisHelper.clearCacheKeys = (keys, debugId) => {
